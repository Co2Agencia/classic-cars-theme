...
const navBtnAutos = document.getElementById("nav-btn-autos")
const navBarBtnAutos = document.getElementById("nav-bar-autos-btn")


/* Agregando funciones al window */
...
class Utils{
    ...
    /* 
        ...
        segun el ancho de pantalla y dispositivo
    */
    checkSlideAmount(){
        ...
        if(!window.mobileCheck() && !window.tabletCheck()){
            ...
            const tabletWidth = window.outerWidth
            const xlTScreen = 1300
            const lTScreen = 850

            if (tabletWidth > xlTScreen) return 3
            else if(tabletWidth > lTScreen && tabletWidth < xlTScreen) return 2
            else return 1
        }
        else{
            return 1
...
class NavBar{
    ...
    */
    mobileMenu(mostrar_o_esconder){
        const navAutos = document.querySelector("#custom-nav-bar .nav-bar-autos")
    ...
    /* 
        - Hace aparecer o desaparecer el nav
    */
    hideShowNav(hide_or_show){
        const Nav = document.getElementById("custom-nav-bar")
        const navAutos = Nav.querySelector(".nav-bar-autos")

        if(hide_or_show == "hide"){
            Nav.style.animationName = "hide-nav"
            navAutos.style.display = "none"
        }
        else{
            Nav.style.animationName = "unset"
            navAutos.style.display = "flex"
        }
    }
}

// Abre el nav al hacer click en el icono
...
// Click en Nav link "autos"
navBtnAutos.addEventListener("click", ()=>{
    const navAutos = document.querySelector("#custom-nav-bar .nav-bar-autos")
    let navCheck = new NavBar().navCheck()

    navAutos.classList.toggle("vanish")

    if(navCheck == "mobile") navMenu.classList.toggle("opacity");

})

navBarBtnAutos.addEventListener("click", ()=>{
    const navAutos = document.querySelector("#custom-nav-bar .nav-bar-autos")
...
// Detecta si se hace scroll para arriba o para abajo
var lastScrollTop = 0;
document.addEventListener("scroll", function(){
   var st = window.pageYOffset || document.documentElement.scrollTop;
   if (st > lastScrollTop){
        new NavBar().hideShowNav("hide")
   } else {
        new NavBar().hideShowNav("show")
   }
...
function crear_cont(el_padre, class_el_hijo, hijos_por_cont, slide_bool = false){
    ...
    let check_odd = hijos_len % 2 // Checkea si el numero es par
    if (hijos_por_cont < 2) check_odd = 0;
    let cant_cont = Math.round(hijos_len / hijos_por_cont) + check_odd
